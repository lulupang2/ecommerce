...
import { API, api, categories, money, optionText } from '@/lib/store';
import { getCurrentUserId, readSession } from '@/lib/session';
...
export default function StoreShell({children}){
  ...
  return <CartContext.Provider value={value}><div className="min-h-screen bg-white pb-16 text-slate-950 md:pb-0">
    ...
    {children}
    <nav className="fixed bottom-0 left-0 right-0 z-30 grid grid-cols-5 border-t bg-white py-2 md:hidden">{[[Home,'홈','/'],[Menu,'카테고리','/shop/'],[Search,'검색','/shop/?focus=search'],[Heart,'찜','/mypage/?tab=wishlist'],[UserRound,'MY','/orders/']].map(([Icon,label,href])=><a key={label} href={href} className="grid place-items-center gap-1 text-[10px] font-bold"><Icon size={19}/>{label}</a>)}</nav>
