...
const adminCatalog = await request('/products?status=all');
...
await request(`/products/${product.id}`, { method: 'PATCH', body: JSON.stringify({ price: originalPrice + 1 }) });
await request(`/products/${product.id}`, { method: 'PATCH', body: JSON.stringify({ price: originalPrice }) });
const inventoryBefore = await request(`/inventory/${product.id}`);
const inventoryList = await request('/inventory');
assert.ok(Array.isArray(inventoryList.items), 'admin inventory list must be available');
await request(`/inventory/${product.id}`, { method: 'PATCH', body: JSON.stringify({ availableQty: inventoryBefore.available_qty }) });
...
await request(`/orders/${order.id}/status`, { method: 'PATCH', body: JSON.stringify({ status: 'confirmed' }) });
