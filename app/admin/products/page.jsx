...
export default function ProductEditorPage() {
  ...
  useEffect(() => {
    ...
    setSession(current);
    if (productId) fetch(`${API}/products/${encodeURIComponent(productId)}`).then(response => response.json()).then(data => { setProduct(data); setPreview(data.image || ''); }).catch(() => setMessage('상품 정보를 불러오지 못했습니다.'));
  }, [productId]);
  ...
  async function submit(event) {
    ...
    try {
      ...
      const response = await fetch(`${API}/products${productId ? `/${productId}` : ''}`, { method: productId ? 'PATCH' : 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${session.accessToken}` }, body: JSON.stringify({ ...product, image, price: Number(product.price), stock: Number(product.stock) }) });
  ...
  if (!session || session.user?.role !== 'admin') return <main className="grid min-h-screen place-items-center bg-slate-100"><div className="rounded-3xl bg-white p-10 text-center"><h1 className="text-2xl font-black">관리자 로그인이 필요합니다.</h1><a href="/login/" className="mt-6 inline-block rounded-xl bg-slate-950 px-5 py-3 text-white">로그인</a></div></main>;

  return <main className="min-h-screen bg-[#f4f6fa] px-5 py-8 text-slate-950 md:px-10"><div className="mx-auto max-w-5xl"><div className="flex items-center justify-between"><a href="/admin/products/manage/" className="inline-flex items-center gap-2 text-sm font-bold"><ArrowLeft size={16}/> 상품 목록</a><span className="text-xs text-slate-400">Lexical WYSIWYG</span></div><header className="py-12"><p className="text-[10px] font-bold tracking-[.24em] text-indigo-600">PRODUCT CONTENT STUDIO</p><h1 className="mt-4 text-5xl font-black tracking-[-.08em]">{productId ? '상품 수정' : '상품 등록'}</h1></header><form onSubmit={submit} className="grid gap-6"><section className="rounded-3xl bg-white p-6"><h2 className="text-xl font-black">기본 정보</h2><div className="mt-5 grid gap-4 md:grid-cols-2">{[['상품명', 'name'], ['브랜드', 'brand'], ['카테고리', 'category'], ['색상·옵션', 'color'], ['판매가', 'price'], ['초기 재고', 'stock']].map(([label, key]) => <label className="grid gap-2 text-xs font-bold" key={key}>{label}<input required={['name', 'brand', 'price', 'stock'].includes(key)} type={['price', 'stock'].includes(key) ? 'number' : 'text'} min={['price', 'stock'].includes(key) ? 0 : undefined} value={product[key] ?? ''} onChange={event => setProduct(current => ({ ...current, [key]: event.target.value }))} className="rounded-xl border p-3 text-sm font-normal" /></label>)}</div><label className="mt-4 grid gap-2 text-xs font-bold">판매 상태<select value={product.status || 'published'} onChange={event => setProduct(current => ({ ...current, status: event.target.value }))} className="rounded-xl border p-3 text-sm font-normal"><option value="published">판매 중</option><option value="hidden">숨김</option><option value="archived">보관</option></select></label></section><section className="rounded-3xl bg-white p-6"><h2 className="text-xl font-black">썸네일 이미지</h2><div onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); chooseFile(event.dataTransfer.files); }} onClick={() => fileRef.current?.click()} className="mt-5 grid min-h-64 cursor-pointer place-items-center overflow-hidden rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50">{preview ? <img src={preview} alt="썸네일 미리보기" className="h-64 w-full object-contain" /> : <div className="text-center"><UploadCloud className="mx-auto text-indigo-600"/><p className="mt-3 font-bold">이미지를 드래그하거나 클릭하세요</p><p className="mt-2 text-xs text-slate-400">JPG, PNG, WEBP</p></div>}<input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={event => chooseFile(event.target.files)} /></div></section><section className="rounded-3xl bg-white p-6"><div className="flex items-center justify-between"><h2 className="text-xl font-black">상세 설명</h2><div className="flex gap-2"><Button type="button" size="sm" variant="outline" onClick={() => editor?.dispatchCommand(FORMAT_TEXT_COMMAND, 'bold')}>굵게</Button><Button type="button" size="sm" variant="outline" onClick={() => editor?.dispatchCommand(INSERT_UNORDERED_LIST_COMMAND, undefined)}>목록</Button></div></div><LexicalComposer initialConfig={editorConfig}><EditorBridge value={product.note} onEditor={setEditor} onHtml={html => setProduct(current => ({ ...current, note: html }))} /></LexicalComposer></section><div className="flex items-center justify-between"><p role="status" className="text-sm text-indigo-600">{message}</p><Button disabled={saving} type="submit">{saving ? <Loader2 className="mr-2 animate-spin" size={16}/> : <Save className="mr-2" size={16}/>}저장</Button></div></form></div></main>;
}
