...
async function init() { await db.wait(); await db.query(`CREATE TABLE IF NOT EXISTS users (id UUID PRIMARY KEY, email TEXT UNIQUE NOT NULL, password_hash TEXT NOT NULL, name TEXT NOT NULL, role TEXT NOT NULL DEFAULT 'customer', created_at TIMESTAMPTZ DEFAULT now())`); const email = process.env.ADMIN_EMAIL; const password = process.env.ADMIN_PASSWORD; if (email && password) { const existing = await db.orm.select({ id: users.id }).from(users).where(eq(users.email, email)).limit(1); if (!existing[0]) await db.orm.insert(users).values({ id: crypto.randomUUID(), email, passwordHash: await bcrypt.hash(password, 10), name: process.env.ADMIN_NAME || 'TECHZONE Admin', role: 'admin' }); } }
function token(user) { return jwt.sign({ sub: user.id, email: user.email, role: user.role }, secret, { expiresIn: '2h' }); }
app.post('/auth/register', async (req, res) => { const { email, password, name } = req.body; const role = 'customer'; if (!email || !password || !name) return res.status(400).json({ code: 'INVALID_INPUT' }); try { const user = { id: crypto.randomUUID(), email, name, role }; await db.orm.insert(users).values({ id: user.id, email, passwordHash: await bcrypt.hash(password, 10), name, role }); await publish('user.registered', { userId: user.id, email, role }); res.status(201).json({ user, accessToken: token(user) }); } catch { res.status(409).json({ code: 'EMAIL_EXISTS' }); } });
...
app.get('/auth/users', requireAuth, requireRole('admin'), async (req, res) => { const rows = await db.orm.select({ id: users.id, email: users.email, name: users.name, role: users.role, createdAt: users.createdAt }).from(users).orderBy(users.createdAt); res.json({ items: rows }); });
