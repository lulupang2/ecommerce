...
import { clearSession, readSession, saveSession } from '@techzone/api-client/session';
...
export default function LoginPage() {
  ...
  async function submit(event) {
    ...
    const body: Record<string, FormDataEntryValue | null> = { email: form.get('email'), password: form.get('password') };
    ...
    try {
      ...
      saveSession(data);
      if (data.user?.role !== 'customer' || data.user?.adminRole) { window.location.href = '/admin/'; return; }
      setSession(data);
