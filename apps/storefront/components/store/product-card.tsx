...
export default function ProductCard({ product, onAdd = null, compact = false }) {
  ...
  return <article className="group min-w-0">
    <div className="relative overflow-hidden rounded-2xl border border-slate-100 bg-[#f4f6f8]">
      {product.discountRate > 0 && <span className="absolute left-3 top-3 z-10 rounded-md bg-rose-500 px-2 py-1 text-[11px] font-black text-white">{product.discountRate}%</span>}
      ...
      <a href={href}><img src={product.image} alt={product.name} className={`w-full object-cover transition duration-500 group-hover:scale-105 ${compact?'aspect-square':'aspect-[.9]'}`}/></a>
    ...
    <div className="pt-4"><p className="text-[10px] font-black tracking-[.16em] text-indigo-600">{product.brand}</p><a href={href} className="mt-1 block truncate text-sm font-bold hover:text-indigo-600">{product.name}</a>
      <div className="mt-2 flex flex-wrap items-baseline gap-2">{product.discountRate > 0 && <b className="text-rose-500">{product.discountRate}%</b>}<strong>{money(product.price)}</strong>{product.listPrice>product.price&&<del className="text-xs text-slate-400">{money(product.listPrice)}</del>}</div>
