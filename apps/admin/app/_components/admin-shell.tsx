...
export default function AdminShell({ children }) {
  ...
  const sidebar = <div className="flex h-full flex-col bg-[#101828] text-slate-200">
    <div className="flex h-18 items-center justify-between border-b border-white/10 px-5">
      {!collapsed && <Link href="/" className="text-lg font-black tracking-[-.08em] text-white">TECH<span className="text-indigo-400">ZONE</span><small className="ml-2 text-[9px] tracking-normal text-slate-500">OPS</small></Link>}
  ...
  return <div className="min-h-screen bg-[#f5f7fb] text-slate-950">
    ...
    <div className={`transition-all duration-200 ${collapsed ? 'lg:pl-20' : 'lg:pl-64'}`}>
      <header className="sticky top-0 z-30 flex h-18 items-center gap-3 border-b border-slate-200 bg-white/95 px-4 backdrop-blur md:px-7">
        ...
        <form className="relative hidden max-w-md flex-1 md:block" onSubmit={event => { event.preventDefault(); const value = new FormData(event.currentTarget).get('q'); if (value) window.location.href = `/admin/products/manage/?q=${encodeURIComponent(String(value))}`; }}><Search size={16} className="absolute left-3 top-2.5 text-slate-400"/><input name="q" placeholder="주문번호, 상품명, SKU 통합 검색" className="w-full rounded-xl bg-slate-100 py-2 pl-10 pr-4 text-sm outline-none ring-indigo-500 focus:ring-2"/></form>
        <div className="ml-auto flex items-center gap-2">
          ...
          <Link href="/alerts/" className="relative rounded-xl border border-slate-200 p-2.5 text-slate-500 hover:bg-slate-50"><Bell size={18}/>{alerts.length > 0 && <span className="absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-rose-500 px-1 text-[10px] font-black text-white">{Math.min(alerts.length, 99)}</span>}</Link>
