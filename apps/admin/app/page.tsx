  ...
  return <main className="p-4 md:p-7 xl:p-9">
    <div className="mx-auto max-w-[1540px]">
      <header className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div><p className="text-xs font-bold tracking-[.16em] text-indigo-600">COMMERCE OPERATIONS</p><h1 className="mt-2 text-3xl font-black tracking-[-.05em] md:text-4xl">오늘의 운영 현황</h1><p className="mt-2 text-sm text-slate-700">매출부터 출고·반품·재고 위험까지 한눈에 확인하세요.</p></div>
...
function DashboardContent({ data }) {
  ...
  return <div className="mt-7 grid gap-6">
    ...
    <section className="grid gap-6 xl:grid-cols-3">
      ...
      <Panel title="처리 대기 업무" description="지금 확인이 필요한 운영 항목"><div className="mt-3 grid gap-3"><QueueLink href="/shipping/?status=ready" icon={Truck} label="출고 지연" value={data.queues.delayedShipments} tone="orange"/><QueueLink href="/returns/?status=requested" icon={RotateCcw} label="반품 처리 대기" value={data.queues.pendingReturns} tone="rose"/><QueueLink href="/inventory/" icon={AlertTriangle} label="안전재고 이하" value={data.queues.inventoryRisk} tone="red"/><QueueLink href="/procurement/?status=approved" icon={ShoppingBag} label="진행 중 발주" value={data.queues.openPurchaseOrders} tone="indigo"/></div></Panel>
    </section>
    <section className="grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
      <Panel title="최근 주문" description="가장 최근 접수된 주문"><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[680px] text-sm"><thead><tr className="border-b text-left text-xs text-slate-600"><th className="pb-3">주문번호</th><th className="pb-3">고객</th><th className="pb-3">상태</th><th className="pb-3 text-right">결제금액</th><th/></tr></thead><tbody className="divide-y">{data.recentOrders.map(order => <tr key={order.order_id}><td className="py-4 font-bold">{order.order_number}</td><td className="py-4 text-slate-500">{order.recipient}</td><td className="py-4"><StatusBadge value={order.status}/></td><td className="py-4 text-right font-semibold">{won(order.total_amount)}</td><td className="py-4 text-right"><Link aria-label={`${order.order_number} 주문 관리로 이동`} href={`/orders/?q=${encodeURIComponent(order.order_number)}`} className="inline-grid h-8 w-8 place-items-center rounded-lg text-indigo-600 hover:bg-indigo-50"><ArrowRight size={15}/></Link></td></tr>)}</tbody></table></div></Panel>
      <Panel title="재고 위험" description="안전재고 이하 SKU"><div className="mt-3 grid gap-2">{data.riskInventory.length ? data.riskInventory.map(item => <Link href={`/inventory/?q=${encodeURIComponent(item.sku || item.name || '')}`} key={item.balance_id} className="flex items-center justify-between rounded-xl bg-slate-50 p-3 hover:bg-rose-50"><div><p className="text-sm font-bold">{item.name || item.sku || '미등록 SKU'}</p><p className="mt-1 text-[11px] text-slate-600">{item.warehouse_name} · {item.sku}</p></div><div className="text-right"><p className="font-black text-rose-700">{item.available_qty}개</p><p className="text-[10px] text-slate-600">안전 {item.safety_qty}</p></div></Link>) : <Empty text="재고 위험 항목이 없습니다."/>}</div></Panel>
    </section>
  </div>;
}

function KpiCard({ label, value, change = undefined, icon: Icon, tone, warning = false }) {
  ...
  return <article className={`rounded-2xl border bg-white p-4 shadow-sm ${warning ? 'border-rose-200' : 'border-slate-200/80'}`}><div className="flex items-start justify-between"><span className={`grid h-9 w-9 place-items-center rounded-xl ${toneClass}`}><Icon size={18}/></span>{change !== undefined && <span className={`flex items-center text-[11px] font-black ${change >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>{change >= 0 ? <ArrowUpRight size={13}/> : <ArrowDownRight size={13}/>} {Math.abs(change)}%</span>}</div><p className="mt-5 text-xs font-semibold text-slate-600">{label}</p><p className="mt-1 whitespace-nowrap text-xl font-black tracking-[-.04em]">{value}</p></article>;
}
function Panel({ title, description, children }) { return <article className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm md:p-6"><div><h2 className="text-base font-black">{title}</h2><p className="mt-1 text-xs text-slate-600">{description}</p></div>{children}</article>; }
...
function TrendTooltip({ active = false, payload = [], label = '' }) { if (!active || !payload?.length) return null; return <div className="rounded-xl border bg-white p-3 text-xs shadow-xl"><p className="mb-2 font-bold">{label}</p>{payload.map(item => <p key={item.name} style={{ color: item.color }}>{item.name}: <b>{item.name === '매출' ? won(item.value) : `${item.value}건`}</b></p>)}</div>; }
function Empty({ text }) { return <div className="rounded-xl bg-slate-50 p-8 text-center text-xs text-slate-600">{text}</div>; }
