...
function DashboardContent({ data }) {
  ...
  return <div className="mt-7 grid gap-6">
    ...
    <section className="grid gap-6 xl:grid-cols-[1.25fr_.75fr]">
      ...
      <Panel title="재고 위험" description="안전재고 이하 SKU"><div className="mt-3 grid gap-2">{data.riskInventory.length ? data.riskInventory.map(item => <Link href={`/inventory/?q=${encodeURIComponent(item.sku || item.name || '')}`} key={item.balance_id} className="flex items-center justify-between rounded-xl bg-slate-50 p-3 hover:bg-rose-50"><div><p className="text-sm font-bold">{item.name || item.sku || '미등록 SKU'}</p><p className="mt-1 text-[11px] text-slate-400">{item.warehouse_name} · {item.sku}</p></div><div className="text-right"><p className="font-black text-rose-600">{item.available_qty}개</p><p className="text-[10px] text-slate-400">안전 {item.safety_qty}</p></div></Link>) : <Empty text="재고 위험 항목이 없습니다."/>}</div></Panel>
    </section>
  </div>;
}
