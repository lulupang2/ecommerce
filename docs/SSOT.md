...
- 무료배송 기준은 80,000원, 기본 배송비는 3,000원이다.
- `TECHZONE10`은 300,000원 이상 주문에 10%, 최대 50,000원까지 고객별 1회 적용한다.
- quote JWT는 10분, 비회원 주문 JWT는 15분 유효하며 주문 전 Catalog의 variant 가격을 재검증한다.
- 가격이 달라지면 `409 PRICE_CHANGED`, 만료 quote는 `410 QUOTE_EXPIRED`를 반환한다.
- 리뷰는 배송 완료된 로그인 구매자만, Q&A는 로그인 사용자만 작성한다.
- 웹 기본 빌드는 `standalone`, `CAPACITOR_BUILD=1` 빌드는 `export`다.
